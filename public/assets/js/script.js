$(document).ready(function () {
    ...
    function init() { //
        var $this = Sidemenu; //
        // KODE YANG DIUBAH: Gunakan 'document' untuk event delegation karena sidebar kini di-include langsung
        $(document).on("click", "#sidebar-menu a", function (e) { //
            var $thisLink = $(this); //

            if ($thisLink.parent().hasClass("submenu")) { //
                e.preventDefault(); //
            }
            ...
            if ($nextUl.length) { //
                if (!$thisLink.hasClass("subdrop")) { //
                    ...
                    $parentUl.find("ul").slideUp(250); //
                    $parentUl.find("a").removeClass("subdrop"); //
