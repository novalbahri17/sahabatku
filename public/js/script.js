$(document).ready(function () {
    ...
    function init() {
        var $this = Sidemenu;
        // Ini adalah kode BARU untuk menggantikan blok di atas di script.js Anda:
$("#sidebar-placeholder").on("click", "#sidebar-menu a", function (e) {
    // Pastikan kita menargetkan elemen yang benar di dalam #sidebar-placeholder
    var $thisLink = $(this); // Link <a> yang diklik

    if ($thisLink.parent().hasClass("submenu")) { // Periksa apakah induk langsungnya adalah <li> dengan kelas 'submenu'
        e.preventDefault(); // Hanya cegah default jika ini adalah link utama submenu
    }

    // Logika untuk membuka dan menutup submenu
    // Cek apakah link ini memiliki submenu (ul setelahnya)
    var $nextUl = $thisLink.next("ul");

    if ($nextUl.length) { // Jika ada <ul> setelah link ini (berarti ini adalah link utama dari sebuah submenu)
        if (!$thisLink.hasClass("subdrop")) { // Jika belum terbuka (tidak punya kelas 'subdrop')
            // Tutup semua submenu lain yang mungkin terbuka di level yang sama
            var $parentUl = $thisLink.closest("ul"); // ul terdekat tempat link ini berada
            $parentUl.find("ul").slideUp(250); // Tutup semua ul di dalamnya
            $parentUl.find("a").removeClass("subdrop"); // Hapus 'subdrop' dari semua link di dalamnya

            // Buka submenu yang diklik
            $nextUl.slideDown(350);
            $thisLink.addClass("subdrop");
        } else { // Jika sudah terbuka (punya kelas 'subdrop')
            $thisLink.removeClass("subdrop");
            $nextUl.slideUp(350);
        }
    }
    // Jika link yang diklik tidak memiliki next("ul"), berarti itu adalah link menu biasa (bukan header submenu),
    // biarkan browser mengikuti href-nya (atau tidak melakukan apa-apa jika href="javascript:void(0);").
});
